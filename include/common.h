...
#undef DUCKSTATION
#endif
#define DUCKSTATION 4
#define VITA 5

//! Memory Regions
#if BUILD == REDUX || BUILD == PS1 || BUILD == PS2_IOP
...
//! Sectors
#if BUILD == PS1 || BUILD == PS2_IOP || BUILD == REDUX || BUILD == DUCKSTATION
...
/* Same as game: angle from +x, low precision [0,255] when high_precision==0. */
int ATan2Scaled(unsigned int x, unsigned int y, int high_precision);

/* Smallest unsigned angular distance between two 8-bit style angles. */
unsigned char CalculateSmallestAngleDistance(int param_1, int param_2);

int SinScaled(unsigned int angle);                                                                   //? Returns sin of angle. Full circle = 0x1000. Output scaled [0,4096].
...
extern AnalogStick _leftAnalogStick;

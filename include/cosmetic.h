...
/* Full preview (REDUX / PS2_DECKARD): cosmetic_menu_preview.c in extra or deckard_ps2m. NTSC: stubs in cosmetic.c */

/* World-space add to _cameraPosition for one DrawSpyro in cosmetic menu; default 0 — set in debugger. */
extern int g_cosmetic_preview_ofs_x;
extern int g_cosmetic_preview_ofs_y;
extern int g_cosmetic_preview_ofs_z;

/* Added to _spyro.angle.yaw each cosmetic preview frame while menu is open; 0 = no spin. */
extern int g_cosmetic_preview_spin_step;

enum CosmeticPreviewMode
{
    COSMETIC_PREVIEW_SPYRO
};

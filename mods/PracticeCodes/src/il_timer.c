...
void ILUpdate() {
    if (il_menu.il_state)
    {
        ...
        else if (il_timer_state == IL_STARTED)
        {
            ...
            {
                if (_effect_ScreenLetterBox == 0xA) // The letter box appears on 0xE, but we are triggering it 2 frames early to appear on the right frame which as accounts for the 4 game frames early that the timer starts normally
                {
                    Timer ilTimer;
                    ilTimer.timer = _globalTimer - ilTimerStart;
                    FramesToTimer(&ilTimer);

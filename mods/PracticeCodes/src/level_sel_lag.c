...
int inventory_timer = 0;

void LevelSelectFunction(int menu, int level_in_menu)
{
    //lag_tests_level_chosen = level;

    if (inventory_timer == 0)
    ...
    // Once inventory menu has opened
    if (inventory_timer == 10)
    {
        _selectMenuOption = menu;
        buttonSelection = level_in_menu;
        levelSelectState = LEVEL_CHOSEN;
    }

    // After level select, reset timer
    if (_gameState == GAMESTATE_GAMEPLAY && inventory_timer > 60)
    {
        inventory_timer = 0;
    }
}

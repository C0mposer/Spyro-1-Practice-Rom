...
def _bin_path(filename: str) -> str:
    return os.path.join(_PROJECT_ROOT, ".config", "output", "bin_files", filename)


def _wad_path() -> str:
    return os.path.join(_PROJECT_ROOT, "build", "spyro1_PracticeCodes", "WAD.WAD")
...
def PatchWadMiscASMPatches():
    target_file = _wad_path()
    offset = int(0x51E9C98)
    replace_file_path = _bin_path("head.bin")
    ...
    replace_file_path = _bin_path("asmheads.bin")
    ...
    offset = int(0x988F68)
    replace_file_path = os.path.join(_SCRIPT_DIR, "bmps", "FlagLODVertexColors.bin")

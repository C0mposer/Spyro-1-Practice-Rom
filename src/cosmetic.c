...
/* Pause-screen clear-color save/restore (grey overlay).
 * Available in all builds — only touches the generic clear-color RGB
 * at 0x80078a50, no build-specific state needed. */
static unsigned char s_practice_menu_saved_clear_r;
static unsigned char s_practice_menu_saved_clear_g;
static unsigned char s_practice_menu_saved_clear_b;
static int s_practice_menu_saved_fill_valid;

void PracticeMenuPauseScreenFillSaveAndGrey(void)
{
	volatile unsigned char* pc = (volatile unsigned char*)0x80078a50;
	if (!s_practice_menu_saved_fill_valid)
	{
		s_practice_menu_saved_clear_r = pc[0];
		s_practice_menu_saved_clear_g = pc[1];
		s_practice_menu_saved_clear_b = pc[2];
		s_practice_menu_saved_fill_valid = 1;
	}
	pc[0] = 0x38;
	pc[1] = 0x38;
	pc[2] = 0x38;
...
void PracticeMenuPauseScreenFillRestore(void)
{
	volatile unsigned char* pc;
	if (!s_practice_menu_saved_fill_valid)
		return;
	pc = (volatile unsigned char*)0x80078a50;
	pc[0] = s_practice_menu_saved_clear_r;
	pc[1] = s_practice_menu_saved_clear_g;
	pc[2] = s_practice_menu_saved_clear_b;
	s_practice_menu_saved_fill_valid = 0;
}

#if !(BUILD == REDUX || BUILD == PS2_DECKARD || BUILD == DUCKSTATION)
/* Practice-menu Spyro preview lives in cosmetic_menu_preview.c (extra / deckard_ps2m) on those builds. */
int g_cosmetic_preview_ofs_x;
int g_cosmetic_preview_ofs_y;
int g_cosmetic_preview_ofs_z;
int g_cosmetic_preview_spin_step;

...
static void SnapILTimer(bool include_loadless)
{
    Timer ilTimer;
...
static bool ShouldSaveGhostForLoadingComplete(void)
{
    return _portalToExitFromInHW && _portalNumber == -1;
}
    ...
    if (il_menu.il_state == true)
    {
        ...
        else if (il_timer_state == IL_STARTED)
        {
            ...
            if (_gameState == GAMESTATE_LOADING)
            {
                SnapILTimer(true);
                #if BUILD == PS2_DECKARD || BUILD == REDUX || BUILD == DUCKSTATION
                if (ShouldSaveGhostForLoadingComplete())
                {
                    GhostOnRunComplete(_globalTimer - ilTimerStart);
                }
                #endif
        ...
        if ((il_timer_state == IL_DISPLAYING) && _gameState != GAMESTATE_EXITING_LEVEL && _gameState != GAMESTATE_INVENTORY && _gameState != GAMESTATE_PAUSED)
        {
            ...
            #if BUILD == PS2_DECKARD || BUILD == REDUX || BUILD == DUCKSTATION
            if (ghost_menu.ghosts_enabled && g_ghost_new_best)
            {
                CapitalTextInfo new_best_info = { SCREEN_LEFT_EDGE + 0x10, 80, 0x1400 };
                DrawTextCapitals("NEW BEST GHOST", &new_best_info, 0xF, MOBY_COLOR_GREEN);
            }
            #endif

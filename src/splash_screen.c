...
void SplashScreen1Hook(void)
{
    if (!has_loaded_splash_1)
    {
        ...
        LoadCdData(265775, SPLASH_1_RAM_AREA, 0x59000, 0, 600);     //Loads kern.bin
        #elif BUILD == PS2_DECKARD
        LoadCdData(265777, SPLASH_1_RAM_AREA, 0x59000, 0, 600);     //Loads kern.bin
        #endif
...
void SplashScreen2Hook(void)
{
    ...
    {
        #if BUILD == PS1 || BUILD == PS2_IOP || BUILD == DUCKSTATION
        LoadCdData(265955, SPLASH_2_RAM_AREA, 0x59000, 0, 600);     //Loads kern.bin
        #elif BUILD == PS2_DECKARD
        LoadCdData(265957, SPLASH_2_RAM_AREA, 0x59000, 0, 600);     //Loads kern.bin
        #endif
